...
_get_area_use_case = GetAreaUseCase(plant_repository)
_update_area_use_case = UpdateAreaUseCase(plant_repository)
_delete_area_use_case = DeleteAreaUseCase(plant_repository)
_list_area_equipment_use_case = ListAreaEquipmentUseCase(plant_repository)
_create_equipment_use_case = CreateEquipmentUseCase(plant_repository)
...
async def update_area(area_id: int, payload: AreaUpdatePayload) -> dict[str, int | str]:
    ...
    updated = _update_area_use_case.execute(area_id, name=update_data.get("nombre"), status=update_data.get("estado"))
    if updated is None:
...
async def delete_area(area_id: int) -> None:
    deleted = _delete_area_use_case.execute(area_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Área no encontrada")
...
async def list_area_equipment(area_id: int) -> list[dict[str, int | str]]:
    area = _get_area_use_case.execute(area_id)
    if area is None:
        raise HTTPException(status_code=404, detail="Área no encontrada")

    equipment = _list_area_equipment_use_case.execute(area_id)
...
async def create_equipment(area_id: int, payload: EquipmentCreatePayload) -> dict[str, int | str]:
    ...
    created = _create_equipment_use_case.execute(area_id, name=payload.nombre, status=payload.estado)
    if created is None:
