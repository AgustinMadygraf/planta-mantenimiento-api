...
_update_system_use_case = UpdateSystemUseCase(plant_repository)
_delete_system_use_case = DeleteSystemUseCase(plant_repository)
...
async def update_system(system_id: int, payload: SystemUpdatePayload) -> dict[str, int | str]:
    ...
    updated = _update_system_use_case.execute(system_id, name=update_data.get("nombre"), status=update_data.get("estado"))
    if updated is None:
...
async def delete_system(system_id: int) -> None:
    deleted = _delete_system_use_case.execute(system_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Sistema no encontrado")
