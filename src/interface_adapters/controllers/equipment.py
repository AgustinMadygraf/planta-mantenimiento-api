...
_get_equipment_use_case = GetEquipmentUseCase(plant_repository)
_update_equipment_use_case = UpdateEquipmentUseCase(plant_repository)
_delete_equipment_use_case = DeleteEquipmentUseCase(plant_repository)
_list_equipment_systems_use_case = ListEquipmentSystemsUseCase(plant_repository)
_create_system_use_case = CreateSystemUseCase(plant_repository)
...
async def update_equipment(equipment_id: int, payload: EquipmentUpdatePayload) -> dict[str, int | str]:
    ...
    updated = _update_equipment_use_case.execute(
        equipment_id,
...
async def delete_equipment(equipment_id: int) -> None:
    deleted = _delete_equipment_use_case.execute(equipment_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Equipo no encontrado")
...
async def list_equipment_systems(equipment_id: int) -> list[dict[str, int | str]]:
    equipment = _get_equipment_use_case.execute(equipment_id)
    if equipment is None:
        raise HTTPException(status_code=404, detail="Equipo no encontrado")

    systems = _list_equipment_systems_use_case.execute(equipment_id)
...
async def create_system(equipment_id: int, payload: SystemCreatePayload) -> dict[str, int | str]:
    ...
    created = _create_system_use_case.execute(equipment_id, name=payload.nombre, status=payload.estado)
    if created is None:
